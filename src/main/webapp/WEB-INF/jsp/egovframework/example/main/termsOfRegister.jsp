...
<html>
  ...
  <body> <%= session.getMaxInactiveInterval()%>
...
③  국립중앙도서관, 국립어린이청소년도서관, 국가전자도서관, 시각장애인용 국가전자도서관, 오아시스, 책바다 국가상호대차서비스의 기존회원은 통합회원으로 전환해야만 해당 서비스를 이용할 수 있습니다.
...
④  도서관은 이용 신청자가 관계 법령에서 규정하는 16세 미만일 경우(국립어린이청소년도서관, 국립세종도서관 제외)에 서비스별 안내에서 정하는 바에 따라 승낙을 보류할 수 있습니다.
⑤  도서관은 회원 가입 절차 완료 이후 제2항 각 호에 따른 사유가 발견된 경우 이용 승낙을 철회할 수 있습니다.
⑥  이용신청 시 설정한 개인정보 유효기간이 지나면 자신의 모든 정보는 시스템 상에서 삭제될 수 있으며, 개인정보 유효기간을 선택하지 않은 자의 개인정보 유효기간은 2년으로 자동 설정됩니다. 
...
③  오프라인 서비스(도서관 이용)는 정기 휴관일 및 공휴일에는 운영하지 않으며, 휴관일은 홈페이지를 통해 공지합니다. 서비스 운영시간은 본관 자료실은 09:00 ~ 18:00, 야간도서관은 18:00 ~ 22:00이며, 디지털도서관은 09:00 ~ 18:00입니다. 다만 야간도서관과 관련하여 국립세종도서관은 별도로 정할 수 있으며, 국립세종도서관 홈페이지를 통해 공지합니다. 
...
본 서비스 이용으로 발생한 분쟁에 대해 소송이 제기되는 경우 대한민국 서울중앙지방법원을 관할 법원으로 합니다.
            ...
            <tr>
              <td>
                <input type="checkbox" name="agreeRegister">이용약관에 동의합니다.<br><br><br><br>
              </td>
            </tr>
            <tr>
              <td>
...
1. 국립중앙도서관은 회원가입, 원활한 고객상담, 각종 서비스 등 기본적인 서비스 제공을 위한 필수정보와 고객 맞춤 서비스 제공을 위한 선택정보로 구분하여 최초 회원가입 시 아래와 같은 최소한의 개인정보를 수집하고 있습니다.
  - 필수항목 : 아이디, 비밀번호, 성명, 생년월일, 성별, 장애구분, 주소, 직업, 전화번호, 휴대전화번호, 이메일 주소, 장애구분(장애인 전용 회원가입 이용시)
  - 선택항목 : 도서관 이용목적, 관심분야, 학교정보(국립어린이청소년도서관 이용 시)
...
회원 이용자의 개인정보는 회원가입일로부터 회원가입 시 설정한 유효기간(미선택시 2년으로 자동 설정) 동안 보유 및 이용되며, 원칙적으로 개인정보의 수집 및 이용목적이 달성되면 지체 없이 파기합니다. 따라서 우리 도서관은 정보주체의 회원탈퇴 요청 시 그 회원의 개인정보를 지체 없이 파기합니다.


정보주체의 권리

가. 국립중앙도서관 회원은 자신의 개인정보 처리와 관련하여 아래와 같은 권리를 가집니다.
...
동의거부권 및 동의 거부에 따른 불이익
 ...
 가입자는 개인정보 수집·이용에 대하여 거부할 수 있는 권리가 있습니다. 단, 이에 대한 동의를 거부할 경우에는 회원가입이 불가능합니다.
            ...
            <tr>
              <td>
                <input type="checkbox" name=agreeRegister>개인정보 수집 및 이용에 대한 안내에 동의합니다.
              </td>
            </tr>
          </table>
...
<br><br>
  <a href="#"> <button type = "button"  class="btn_order" style="margin:10px;" onclick="agreeAndRegisterMember()">회 원 가 입</button></a>
  <a href="#"> <button type = "button"  class="btn_cancel1" style="margin:10px;" onclick="cancelRegisterMember()">취 소 하 기</button></a>
	...
	function agreeAndRegisterMember() {

		if ($("input:checkbox[name='agreeRegister']:checked").length != 2) {

			alert("이용약관을 동의하셔야 회원가입을 진행할 수 있습니다.");
		} else {
	
			$("#registerFrm").attr("action", "registerMemberView.do");
			$("#registerFrm").submit();
		}
